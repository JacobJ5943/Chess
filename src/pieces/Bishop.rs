...
impl Bishop {
    fn check_if_piece_on_location(&self, x_coord: usize, y_coord: usize, quick_board: &Vec<Vec<QuickPiece>>) -> bool{
        let quick_piece_row = quick_board.get(x_coord);
        true
    }
...
impl PieceMove for Bishop {
    ...
    fn can_move(&self, x_coord: usize, y_coord: usize, quick_board: &Vec<Vec<QuickPiece>>) -> bool {
        ...
        if self.check_if_piece_on_location(x_coord,y_coord,quick_board) {
